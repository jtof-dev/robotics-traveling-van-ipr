...
volatile float current_force = 0.0f;
...
void loop() {
  ...
  if (shared_state == STATE_BALANCING) {
    if (millis() - lastPrint > 100) {
      lastPrint = millis();
      Serial.printf("Ang: %6.2f | F: %6.1f | Spd: %6.2f | Enc: %ld\r\n", 
                    current_angle, current_force, current_speed, current_encoder);
    }
  }
...
void setup1() {
  ...
  // PID init
  cartPID = new PID(&angle_val, &output_force, &set_point, CART_KP, CART_KI, CART_KD, REVERSE);
...
void loop1() {
  ...
  if (current_state == STATE_NEEDS_RESET || !system_running) {
    ...
    cartPID->Compute();
    float force = output_force;

    if (fabs(angle_val - set_point) < 2.0f) {
      force = 0.0f; // deadband for perfect balance
    }

    // physics pipeline: Force -> Accel -> Speed -> PWM
    float acceleration = force / CART_MASS;
    local_speed += (acceleration * dt);
    local_speed *= 0.98f; // damping
